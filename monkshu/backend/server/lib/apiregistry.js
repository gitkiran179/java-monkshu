...
function initSync() {
	...
	for (const appObj of apps) {
		...
		if (fs.existsSync(`${CONSTANTS.APPROOTDIR}/${app}/conf/apiregistry.decoders.json`)) decoderPathAndRoots.push(
			{path: `${CONSTANTS.APPROOTDIR}/${app}/conf/apiregistry.decoders.json`, root: appRoot});
